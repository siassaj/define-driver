...
function startFunction(sourceListener, sink$, opts) {
  sink$.addListener({
    next: body => {
      const namespace = body._namespace || []
      const category = body.category

      const result$ = opts.callbacks.sinkNext(body) || xs.empty()
...
function isolateSource(source, scope, sourceClass, sourceOptions, sourceConfig) {
  ...
  return new sourceClass(newUnscopedResult$$, sourceOptions, sourceConfig)
}
    ...
    const driver = (sink$) => {
      const producer = {
        start: function(sourceListener) {
          return  startFunction.call(this, sourceListener, sink$, opts)
        },
        stop:  function() { stopFunction.call(this, sink$) },
        sinkListener: null
      }

      const ownAndChildResults$$ = adapt(xs.create(producer))
      return new opts.source(ownAndChildResults$$, options, opts.config)
    }
